...
function SinglePageContainer({
  ...
  return (
    <div className="flex flex-col space-y-10 max-w-7xl mx-auto px-1">
      <div className="flex items-center justify-between">
        <div className="flex space-x-3 lg:space-x-5 items-center h-28">
          ...
          <div className="flex flex-col justify-center space-y-3 h-full">
            ...
            <h2 className="text-light text-2xl">{data.name}</h2>
          </div>
        </div>
        {data.rating && (
          <div className="flex space-x-2 text-light">
                  ...
                  stars.map((_, idx) => (
                    <div key={idx}>
                      {idx !== stars.length - 1 ? (
                        ...
                        <FaStarHalf className="text-light" />
                      )}
                    </div>
                  ))}
              </div>
          ...
          <div className="absolute h-full w-full flex flex-col items-center justify-center p-5 space-y-3">
            ...
            >
              <Link
                href={
                  type === "booster"
                    ? `/boosting?booster=${data._id}`
                    : `/coaching?coach=${data._id}`
                }
              >{`Choose ${data.name} as Your ${type}`}</Link>
            </Button>
          </div>
        </div>
        <div className="flex lg:flex-col h-full w-full space-x-1 lg:space-x-0 lg:w-1/3 lg:space-y-5 justify-between">
          ...
          <div className="bg-dark rounded-xl border-primary border w-full p-2 lg:p-5 text-center space-y-5 shadow-base shadow-primary flex flex-col items-center justify-center">
            ...
            <p className="text-xs lg:text-xl">
              {type === "booster" ? data.boostingSince : data.coachingSince}
            </p>
          </div>
          <div className="bg-dark rounded-xl border-primary border w-full p-2 lg:p-5 text-center space-y-5 shadow-base shadow-primary flex flex-col items-center justify-center">
      ...
      {reviews.length && (
        <div className="w-full text-light space-y-10">
          <h1 className="font-bold text-3xl">Read Reviews</h1>
          <div className="w-full pb-5 overflow-x-auto flex space-x-5 max-w-7xl">
            {reviews.map((review, idx) => (
              <div
                key={idx}
                className="rounded-xl border border-primary bg-dark p-1 sm:p-7 flex items-center justify-center shadow-base shadow-primary"
              >
                <div className="w-[85vw] lg:w-[30rem] h-full space-y-7 flex flex-col items-center justify-center">
                  ...
                  <div className="w-full h-2/3 text-light space-y-2">
